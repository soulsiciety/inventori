...
class C_barang extends CI_Controller{
    ...
    function input()
    {
        ...
        $data['no_bmn']             = $this->input->post('no_bmn');
        $data['nama_barang']        = $this->input->post('nama_barang');
        $data['minimum_stok']       = $this->input->post('minimum_stok');
        ...
        if ($this->form_validation->run() == FALSE)
        {
            ...
            $data['jenis']      = $this->m_jenis_barang->all_semua()->result();
            $this->load->view('header',$data);
    ...
    function edit_stok($id_b_masuk)
    {
        ...
        if($this->form_validation->run()==true)
        {
            ...
            redirect('c_barang/detail/'.$id_barang);
        }
        else
        {
     ...
     function delete_stok()
    {
        $kode=$this->input->post('kode');
       // $data['barang'] = $this->m_barang->get_detail_stok($id_b_masuk)->row_array();
        $this->m_barang->delete_stok($kode);
        //redirect('c_barang/detail/'.$barang['id_barang']);
    }
	
	function get_kat(){
		$data="";
		$id_jenis = $this->input->post('id_jenis');
		$kategori = $this->m_kategori_barang->get_kat($id_jenis)->result();
		$data .= "<option value=''>Pilih kategori</option>";
		foreach($kategori as $row){
			$data .= "<option value='$row->id_kategori'>$row->nama_kategori</option>";
		}
		echo $data;
	}
    function get_kla(){
        ...
        $klasifikasi = $this->m_klasifikasi_barang->get_kla($id_kategori)->result();
        $data .= "<option value=''>Pilih klasifikasi</option>";
        foreach($klasifikasi as $row){
            $data .= "<option value='$row->id_klasifikasi'>$row->nama_klasifikasi</option>";
        }
        echo $data;
    }
    ...
    function _set_rules2(){
        $this->form_validation->set_rules('tgl_entry', 'tgl_entry', 'trim|required');
        $this->form_validation->set_rules('qty_masuk', 'Jumlah Barang Masuk', 'trim|required');
        $this->form_validation->set_error_delimiters("<div class='alert alert-danger'>","</div>");
    }
